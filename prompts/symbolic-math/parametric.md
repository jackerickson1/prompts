...
Output: Single executable script, minimal but complete, include the appendix metadata block at the end (mandatory for plain‑text Live Script recognition):
    ...
    %---
```
2. **Symbolic Math Toolbox skill:** For best results, load the [Symbolic Math Toolbox skill](https://github.com/matlab/skills/blob/main/skills/matlab-symbolic-math/SKILL.md)
...
6. Save the symbolic wn and zeta expressions and the evaluation grid data to a MAT‑file
```
...
6. Save the symbolic Greek expressions and reference parameter values to a MAT‑file
```
